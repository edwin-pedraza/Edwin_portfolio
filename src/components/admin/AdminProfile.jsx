    ...
    <div className="p-4">
      ...
      {loading ? (
        ...
        <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white/5 p-5 rounded-xl border border-white/10 shadow">
          <input className="px-3 py-2 rounded bg-white/10" placeholder="Full name" value={form.full_name} onChange={e=>setForm({...form, full_name:e.target.value})} />
          <input className="px-3 py-2 rounded bg-white/10" placeholder="GitHub URL" value={form.github_url} onChange={e=>setForm({...form, github_url:e.target.value})} />
          <input className="px-3 py-2 rounded bg-white/10 md:col-span-2" placeholder="LinkedIn URL" value={form.linkedin_url} onChange={e=>setForm({...form, linkedin_url:e.target.value})} />
          <div className="md:col-span-2">
            <ImageUploader label="Photo" pathPrefix="profile" value={form.photo_url} onChange={(url)=>setForm({...form, photo_url:url})} />
          </div>
          <textarea className="px-3 py-2 rounded bg-white/10 md:col-span-2" rows={6} placeholder="About text (one paragraph per line)" value={form.about_text} onChange={e=>setForm({...form, about_text:e.target.value})} />
